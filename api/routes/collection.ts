...
import type { CollectionResponseType, ListCollectionsResponseType } from '../lib/types.js';
import {
...
export default async function router(schema: Schema, config: Config) {
        ...
        try {
            ...
            if (!req.auth || !req.auth.level || req.auth.level !== 'sponsor') {
                for (const c of collections) {
                    delete (c as Record<string, unknown>).s3;
                    delete (c as Record<string, unknown>).processed_s3;
                }
            }

            return res.json(collections as unknown as ListCollectionsResponseType);
        } catch (err) {
    ...
    await schema.get('/collections/:collection', {
        ...
        try {
            ...
            return res.json(collection as unknown as CollectionResponseType);
        } catch (err) {
            return Err.respond(err, res);
        }
    });
