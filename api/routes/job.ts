...
import type { JobResponseType, ListJobsResponseType, SingleDeltaResponseType, SingleLogResponseType } from '../lib/types.js';
import {
    ListJobsQuery,
    ListJobsResponse,
...
export default async function router(schema: Schema, config: Config) {
    ...
    await schema.get('/job', {
        ...
        try {
            ...
            return res.json({
                total: list.total,
                jobs: list.items,
            } as unknown as ListJobsResponseType);
        } catch (err) {
    ...
    await schema.get('/job/:job', {
        ...
        try {
            ...
            return res.json(job as unknown as JobResponseType);
        } catch (err) {
            return Err.respond(err, res);
        }
    });
    ...
    await schema.post('/job/:job/rerun', {
        ...
        try {
            ...
            const job = await config.models.Job.from(req.params.job);
            const run = await config.models.Run.from(Number(job.run));

            const new_run = await config.models.Run.generate({
                live: !!run.live,
            });

            return res.json(await config.models.Run.populate(Number(new_run.id), [{
                source: String(job.source),
                layer: String(job.layer),
                name: String(job.name),
                license: job.license,
            }] as unknown as Parameters<typeof config.models.Run.populate>[1]));
        } catch (err) {
            return Err.respond(err, res);
        }
    });
    ...
    await schema.get('/job/:job/delta', {
        ...
        try {
            ...
            return res.json(delta as unknown as SingleDeltaResponseType);
        } catch (err) {
            return Err.respond(err, res);
        }
    });
        ...
        try {
            ...
            if (!req.query.dl) {
                if (!req.query.format || req.query.format === 'json') {
                    return res.json(log as unknown as SingleLogResponseType);
                } else {
                    return res.send(log as unknown as SingleLogResponseType);
                }
            } else {
                res.attachment(`log-${req.params.job}.${req.query.format || 'json'}`);
                res.send(log as unknown as SingleLogResponseType);
            }
        } catch (err) {
            return Err.respond(err, res);
        }
